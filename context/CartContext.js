...
export function CartProvider({ children }) {
  ...
  useEffect(() => {
    const savedCart = localStorage.getItem('powerbottle-cart')
    if (savedCart) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('powerbottle-cart', JSON.stringify(state.items))
  }, [state.items])
  ...
  const updateQuantity = (productId, variant = 'default', quantity) => {
    dispatch({
